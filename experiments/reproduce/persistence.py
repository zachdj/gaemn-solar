...
def historic_value():
    ...
    historic_avgs = compute_historic_averages()
...
def current_and_historic():
    """ Compute MAE using the average of the historic-value and current-value persistence models """
    ...
    logger.debug('Computing MAE for combined average-historic persistence model')
    hr_1_query = "SELECT sr.dateandtime, sr.day as computed_day, (sr.time+100) as computed_time, " \
                    "sr.sr AS current_val, sr.sr1 AS true_val FROM solarradiation sr " \
                 "INNER JOIN gribprediction gp ON gp.SolarRadiationID=sr.id AND (gp.Datasource='rap' OR gp.Datasource='rapinterpolation') " \
                 "AND gp.HoursAhead=1 AND gp.cell='' and gp.SiteID=sr.siteID " \
                 "WHERE sr.dateandtime BETWEEN '2011-06-22' AND '2012-04-30 23:45:00' and sr.SiteID IN %s " \
                 "ORDER BY sr.dateandtime " \
    ...
    hr_24_query = "SELECT sr.dateandtime, (sr.day+1) as computed_day, sr.time as computed_time, " \
                    "sr.sr AS current_val, sr.sr24 AS true_val FROM solarradiation sr " \
                  "INNER JOIN gribprediction gp ON gp.SolarRadiationID=sr.id AND (gp.Datasource='NAM' OR gp.Datasource='NAMinterpolation') " \
                  "AND gp.HoursAhead=24 AND gp.cell='' and gp.SiteID=sr.siteID " \
                  "WHERE sr.dateandtime BETWEEN '2003-01-01' AND '2006-01-01' and sr.SiteID IN %s " \
                  "ORDER BY sr.dateandtime " \
    ...
    for site in gaemn_sites:
        ...
        for record in cursor:
            ...
            key = f'{day}-{time}'
            true_val = float(record['true_val'])
            current_value = float(record['current_val'])

            y_true.append(true_val)
            if key in historic_avgs:
                y_pred.append(historic_avgs[key]*0.5 + current_value*0.5)
            else:
                y_pred.append(0 + current_value*0.5)
    ...
    logger.info('MAEs using average current-value and historic-value persistence model')
    logger.info(table.draw())


def compute_historic_averages():
    # open connection
    db_config = solardata.config.to_dict()
    cnx = mysql.connector.connect(**db_config)
    cursor = cnx.cursor(dictionary=True)
